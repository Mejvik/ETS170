...
\item[Release plan]
The release plan was done by first assign a cost to every feature so that the cost would be as objective as possible. The list of features was then sorted according to the result from the 100$ method from the key costumer. The top five features was picked for the first release to fulfill the key costumer’s most important needs. Results from the end user questionnaire was then taking into account and added to the release plan accordingly. For every feature that was added the cost was taken under consideration and dependencies was analyzed so that they will be implemented in an order that works. 
