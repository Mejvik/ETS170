...
\subsection{Prioritization}
\begin{description}
\item[Stakeholders] Some parts of the prioritization process of the stakeholders were relatively easy, whereas others were more difficult. The stakeholders identified as the most important are the ones that would in some way interact with the system. For example the mobile users were considered as the most important stakeholder as the product is entirely dependent on that they are willing to use it. A major difficulty was to handle the situation when one stakeholder with a low priority had a strong opinion about one aspect of the system that collided with a weaker opinion of a higher prioritized stakeholder. For example, the developers had strong opinions on what should happen when a user tries to send a postcard and the internet connection does not work. 

A learning outcome is that prioritization should have been done at an earlier stage of the project, especially of the stakeholders. For this project though, it did not cause any major issues since the group members all had similar views despite the lack of a formal prioritization. 

\item[Release plan]
The first problem that occurred for the release plan was to establish the cost estimates. Different members had different cost estimates for the features as a result of previous experiences in mobile application development. Some had experience developing for the android platform while others had developed for iOS. It is reasonable that features are more or less difficult to implement on one platform than another, but it was decided to use an average cost as an estimate. The dependency problem was easier to solve as there were not many dependencies to begin with and the dependencies that were there had a low cost and could be added easily. To handle constraints in the release plan it was decided that the cost of different releases should be quite evenly distributed while taking into consideration which features were essential, important or "good to have".

There is a tool to help the specification of requirements and release planning, called reqT, that was recommended to use in this project. After the cost and benefit were estimated, the data was put into reqT to try and solve the optimization problem. While working with reqT it was learned just how hard the optimization problem can be to solve. In fact reqT could not solve it within a reasonable time span without manually modifying the problem. Due to inexperience with the tool it was decided to abandon it and do the release planning manually. In this project this was reasonable since the amount of requirements was manageable. 

\item[Features] The \$100 method was considered a good technique when prioritizing features, however some things could have been done differently. For example, letting the customer grade a basic feature essential to the product, against a feature that could improve the user experience is not a good technique. If the basic features would be graded separately from the external features, the results would be more useful.

In addition to the \$100 method, the replies to the questionnaire were also used. Although the users did not have the opportunity to grade all the final features, their input was considered important and was therefore taken into account.

One of the difficulties was constantly changing requirements, which meant that some of the prioritization that had been done was not entirely accurate anymore.
